...
class AugmentTransposeAxes(unittest.TestCase):
    ...
    def test_transpose_axes(self):
        ...
        for i in range(n_iter):
            data_out, seg_out = augment_transpose_axes(
                self.data_3D, self.seg_3D, axes=(1, 0)
            )

            if np.array_equal(data_out, np.swapaxes(self.data_3D, 1, 2)):
                tmp += 1
        self.assertAlmostEqual(tmp, n_iter / 2.0, delta=10)
...
class AugmentResize(unittest.TestCase):
    ...
    def test_resize(self):
        data_resized, seg_resized = augment_resize(
            self.data_3D, self.seg_3D, target_size=15
        )
        ...
        self.assertTrue(
            all(
                (data_resized.shape[i] == 15 and seg_resized.shape[i] == 15)
                for i in range(1, len(data_resized.shape))
            )
        )

    def test_resize2(self):
        data_resized, seg_resized = augment_resize(
            self.data_3D, self.seg_3D, target_size=(7, 5, 6)
        )
...
class AugmentRot90(unittest.TestCase):
    ...
    def test_rotation_checkerboard(self):
        ...
        for i in range(n_iter):
            d_r, _ = augment_rot90(
                np.copy(data_2d_checkerboard), None, num_rot=[4, 1], axes=[0, 1]
            )
    ...
    def test_rotation(self):
        data_rotated, seg_rotated = augment_rot90(
        ...
        for i in range(self.data_3D.shape[1]):
            self.assertTrue(
                np.array_equal(
                    self.data_3D[:, i, :, :], np.flip(data_rotated[:, :, i, :], axis=1)
                )
            )
            self.assertTrue(
                np.array_equal(
                    self.seg_3D[:, i, :, :], np.flip(seg_rotated[:, :, i, :], axis=1)
                )
            )
    ...
    def test_randomness_rotation_axis(self):
        ...
        for j in range(100):
            ...
            if np.array_equal(
                self.data_3D[:, 0, :, :], np.flip(data_rotated[:, :, 0, :], axis=1)
            ):
                tmp += 1
    ...
    def test_rotation_list(self):
        num_rot = [1, 3]
        data_rotated, seg_rotated = augment_rot90(
            np.copy(self.data_3D), np.copy(self.seg_3D), num_rot=num_rot, axes=[0, 1]
        )
        tmp = 0
        for i in range(self.data_3D.shape[1]):
            # check for normal and inverse rotations
            normal_rotated = np.array_equal(
                self.data_3D[:, i, :, :], data_rotated[:, :, -i - 1, :]
            )
            inverse_rotated = np.array_equal(
                self.data_3D[:, i, :, :], np.flip(data_rotated[:, :, i, :], axis=1)
            )
            if normal_rotated:
                tmp += 1
            ...
            self.assertTrue(
                np.array_equal(self.seg_3D[:, i, :, :], seg_rotated[:, :, -i - 1, :])
                or np.array_equal(
                    self.seg_3D[:, i, :, :], np.flip(seg_rotated[:, :, i, :], axis=1)
                )
            )
    ...
    def test_randomness_rotation_number(self):
        ...
        for j in range(n_iter):
            ...
            normal_rotated = np.array_equal(
                self.data_3D[:, 0, :, :], data_rotated[:, :, -1, :]
            )
            if normal_rotated:
                tmp += 1
        self.assertAlmostEqual(tmp, n_iter / 2.0, delta=20)
