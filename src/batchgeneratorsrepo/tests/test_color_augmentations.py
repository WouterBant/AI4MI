...
class TestAugmentContrast(unittest.TestCase):
    def setUp(self):
        ...
        self.d_3D = augment_contrast(
            self.data_3D,
            contrast_range=self.factor,
            preserve_range=False,
            per_channel=False,
        )
        self.d_2D = augment_contrast(
            self.data_2D,
            contrast_range=self.factor,
            preserve_range=False,
            per_channel=False,
    ...
    def test_augment_contrast_3D(self):
        ...
        idx1 = np.where(
            self.data_3D > mean
        )  # where the data is greater than mean value
        ...
        self.assertTrue(
            np.all(
                np.logical_and(
                    self.d_3D[idx0] >= contrast_lower_limit_0,
        ...
        self.assertTrue(
            np.all(
                np.logical_and(
                    self.d_3D[idx1] >= contrast_lower_limit_1,
    ...
    def test_augment_contrast_2D(self):
        ...
        self.assertTrue(
            np.all(
                np.logical_and(
                    self.d_2D[idx0] >= contrast_lower_limit_0,
        ...
        self.assertTrue(
            np.all(
                np.logical_and(
                    self.d_2D[idx1] >= contrast_lower_limit_1,
...
class TestAugmentBrightness(unittest.TestCase):
    def setUp(self):
        ...
        self.d_3D_per_channel = augment_brightness_additive(
            np.copy(self.data_input_3D), mu=100, sigma=10, per_channel=True
        )
        self.d_3D = augment_brightness_additive(
            np.copy(self.data_input_3D), mu=100, sigma=10, per_channel=False
        )

        self.d_2D_per_channel = augment_brightness_additive(
            np.copy(self.data_input_2D), mu=100, sigma=10, per_channel=True
        )
        self.d_2D = augment_brightness_additive(
            np.copy(self.data_input_2D), mu=100, sigma=10, per_channel=False
        )

        self.d_3D_per_channel_mult = augment_brightness_multiplicative(
            np.copy(self.data_input_3D),
            multiplier_range=self.multiplier_range,
            per_channel=True,
        )
        self.d_3D_mult = augment_brightness_multiplicative(
            np.copy(self.data_input_3D),
            multiplier_range=self.multiplier_range,
            per_channel=False,
        )

        self.d_2D_per_channel_mult = augment_brightness_multiplicative(
            np.copy(self.data_input_2D),
            multiplier_range=self.multiplier_range,
            per_channel=True,
        )
        self.d_2D_mult = augment_brightness_multiplicative(
            np.copy(self.data_input_2D),
            multiplier_range=self.multiplier_range,
            per_channel=False,
        )
    ...
    def test_augment_brightness_additive_3D(self):
        ...
        self.assertTrue(
            len(np.unique(add_factor.round(decimals=8))) == 1,
            "Added brightness factor is not equal for all channels",
        )

        add_factor = self.d_3D_per_channel - self.data_input_3D
        self.assertTrue(
            len(np.unique(add_factor.round(decimals=8))) == self.data_input_3D.shape[0],
    ...
    def test_augment_brightness_additive_2D(self):
        ...
        self.assertTrue(
            len(np.unique(add_factor.round(decimals=8))) == self.data_input_2D.shape[0],
            "Added brightness factor is not different for each channels",
        )
...
class TestAugmentGamma(unittest.TestCase):
    def setUp(self):
        ...
        self.d_3D = augment_gamma(
            np.copy(self.data_input_2D), gamma_range=(0.2, 1.2), per_channel=False
        )

    def test_augment_gamma_3D(self):
        self.assertTrue(
            self.d_3D.min().round(decimals=3)
            == self.data_input_3D.min().round(decimals=3)
            and self.d_3D.max().round(decimals=3)
            == self.data_input_3D.max().round(decimals=3),
