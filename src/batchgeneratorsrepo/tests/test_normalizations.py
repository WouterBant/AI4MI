...
import numpy as np
from batchgenerators.augmentations.normalizations import (
    range_normalization,
...
class TestNormalization(unittest.TestCase):
    ...
    def test_range_normalization_per_channel(self):
        ...
        for b in range(data.shape[0]):
            for c in range(data.shape[1]):
                self.assertAlmostEqual(
                    data_normalized[b, c, :, :].max(),
                    rng1[1],
                    msg="not scaled to correct max range limit",
                )
                self.assertAlmostEqual(
                    data_normalized[b, c, :, :].min(),
                    rng1[0],
        ...
        for b in range(data.shape[0]):
            for c in range(data.shape[1]):
                self.assertAlmostEqual(
                    data_normalized2[b, c, :, :].max(),
                    rng2[1],
                    msg="not scaled to correct max range limit",
                )
                self.assertAlmostEqual(
                    data_normalized2[b, c, :, :].min(),
                    rng2[0],
                    msg="not scaled to correct min range limit",
                )
        ...
        for b in range(data3.shape[0]):
            for c in range(data3.shape[1]):
                self.assertAlmostEqual(
                    data_normalized3[b, c, :, :].max(),
                    rng3[0],
                    msg="not scaled to correct max range limit",
                )
                self.assertAlmostEqual(
                    data_normalized3[b, c, :, :].min(),
                    rng3[0],
                    msg="not scaled to correct min range limit",
                )
    ...
    def test_range_normalization_whole_image(self):
        ...
        data_normalized1 = range_normalization(
            data, rnge=rng1, per_channel=False, eps=0
        )
        ...
        for b in range(data.shape[0]):
            self.assertAlmostEqual(
                data_normalized1[b].min(),
                rng1[0],
                delta=1e-4,
                msg="not scaled to correct min range limit",
            )
            self.assertAlmostEqual(
                data_normalized1[b].max(),
                rng1[1],
                delta=1e-4,
                msg="not scaled to correct max range limit",
            )
            self.assertEqual(
                np.unravel_index(
                    np.argmax(data_normalized1[b], axis=None), data_normalized1[b].shape
                )[0],
                0,
                msg="max not in the right channel",
            )
        print("Test normalization with range [0,1]. [DONE]")
        ...
        for b in range(data.shape[0]):
            self.assertAlmostEqual(
                data_normalized2[b].min(),
                rng2[0],
                delta=1e-4,
                msg="not scaled to correct min range limit",
            )
            self.assertAlmostEqual(
                data_normalized2[b].max(),
                rng2[1],
                delta=1e-4,
                msg="not scaled to correct max range limit",
            )
            self.assertEqual(
                np.unravel_index(
                    np.argmax(data_normalized2[b], axis=None), data_normalized1[b].shape
                )[0],
                0,
                msg="max not in the right channel",
            )
        ...
        for b in range(data3.shape[0]):
            self.assertAlmostEqual(
                data_normalized3[b].min(),
                rng3[0],
                delta=1e-4,
                msg="not scaled to correct min range limit",
            )
            self.assertAlmostEqual(
                data_normalized3[b].max(),
                rng3[0],
                delta=1e-4,
                msg="not scaled to correct max range limit",
            )
    ...
    def test_zero_mean_unit_variance_per_channel(self):
        ...
        data_normalized1 = zero_mean_unit_variance_normalization(
            data, per_channel=True, epsilon=0
        )
        data_normalized2 = zero_mean_unit_variance_normalization(
            data2, per_channel=True
        )
        data_normalized3 = zero_mean_unit_variance_normalization(
            data3, per_channel=True
        )
        ...
        for b in range(data.shape[0]):
            for c in range(data.shape[1]):
                self.assertAlmostEqual(
                    data_normalized1[b, c, :, :].mean(), 0, msg="mean not zeros"
                )
                self.assertAlmostEqual(
                    data_normalized1[b, c, :, :].std(), 1, msg="std not 1"
                )
        ...
        for b in range(data2.shape[0]):
            for c in range(data2.shape[1]):
                self.assertAlmostEqual(
                    data_normalized2[b, c, :, :].mean(), 0, msg="mean not zeros"
                )
                self.assertAlmostEqual(
                    data_normalized2[b, c, :, :].std(), 0, msg="std not 1"
                )
    ...
    def test_zero_mean_unit_variance_whole_image(self):
        ...
        data_normalized1 = zero_mean_unit_variance_normalization(
            data, per_channel=False, epsilon=0
        )
        data_normalized2 = zero_mean_unit_variance_normalization(
            data2, per_channel=False
        )
        data_normalized3 = zero_mean_unit_variance_normalization(
            data3, per_channel=False
        )
    ...
    def test_cut_off_outliers_per_channel(self):
        ...
        for b in range(data.shape[0]):
            for c in range(data.shape[1]):
                self.assertAlmostEqual(
                    data_normalized[b, c].min(), 1.0, msg="Lowe outlier not removed."
                )
                self.assertAlmostEqual(
                    data_normalized[b, c].max(), 1.0, msg="Upper outlier not removed."
                )
    ...
    def test_cut_off_outliers_whole_image(self):
        ...
        for b in range(data.shape[0]):
            self.assertAlmostEqual(
                data_normalized[b].min(), 1.0, msg="Lowe outlier not removed."
            )
            self.assertAlmostEqual(
                data_normalized[b].max(), 1.0, msg="Upper outlier not removed."
            )
