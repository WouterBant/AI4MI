...
if __name__ == "__main__":
    ...
    assert (
        batches == len(cifar_dataset) / batch_size
    )  # this assertion only holds if len(datset) is divisible by
    # batch size
    ...
    dl = HighPerformanceCIFARLoader(
        cifar_dataset_as_arrays, batch_size, num_workers, 1
    ...
    assert (
        batches == len(cifar_dataset_as_arrays[0]) / batch_size
    )  # this assertion only holds if len(datset) is
