...
def sample_scalar(scalar_type: ScalarType, *args):
    if isinstance(scalar_type, (int, float)):
        ...
        assert scalar_type[0] <= scalar_type[1], (
            "if list is provided, first entry must be smaller or equal than second entry, "
        ...
        raise RuntimeError(
            "Unknown type: %s. Expected: int, float, list, tuple, callable",
            type(scalar_type),
        )
