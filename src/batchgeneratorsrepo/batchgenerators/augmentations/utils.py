...
def center_crop_3D_image(img, crop_size):
    ...
    if type(crop_size) not in (tuple, list):
        ...
        assert (
            len(center_crop) == len(img.shape)
        ), "If you provide a list/tuple as center crop make sure it has the same len as your data has dims (3d)"
    return img[
        int(center[0] - center_crop[0] / 2.0) : int(center[0] + center_crop[0] / 2.0),
        int(center[1] - center_crop[1] / 2.0) : int(center[1] + center_crop[1] / 2.0),
        int(center[2] - center_crop[2] / 2.0) : int(center[2] + center_crop[2] / 2.0),
    ]
...
def center_crop_3D_image_batched(img, crop_size):
    ...
    if type(crop_size) not in (tuple, list):
        ...
        center_crop = crop_size
        assert (
            len(center_crop) == (len(img.shape) - 2)
        ), "If you provide a list/tuple as center crop make sure it has the same len as your data has dims (3d)"
    return img[
        :,
        :,
...
def random_crop_3D_image(img, crop_size):
    ...
    if crop_size[2] < img.shape[2]:
        ...
        raise ValueError(
            "crop_size[2] must be smaller or equal to the images z dimension"
        )

    return img[
        lb_x : lb_x + crop_size[0],
...
def resize_image_by_padding_batched(image, new_shape, pad_value=None):
    ...
    if len(shape) == 2:
        res = (
            np.ones(
                (image.shape[0], image.shape[1], new_shape[0], new_shape[1]),
                dtype=image.dtype,
            )
            * pad_value
        )
...
def create_random_rotation(
    ...
    return create_matrix_rotation_x_3d(
        np.random.uniform(*angle_x),
        create_matrix_rotation_y_3d(
            np.random.uniform(*angle_y),
            create_matrix_rotation_z_3d(np.random.uniform(*angle_z)),
        ),
    )
...
def convert_seg_to_bounding_box_coordinates(
    data_dict, dim, get_rois_from_seg_flag=False, class_specific_seg_flag=False
):
    """
    ...
    for b in range(data_dict["seg"].shape[0]):
        ...
        if np.sum(data_dict["seg"][b] != 0) > 0:
            ...
            rois = np.array(
                [(clusters == ii) * 1 for ii in range(1, n_cands + 1)]
            ...
            for rix, r in enumerate(rois):
                if np.sum(r != 0) > 0:  # check if the lesion survived data augmentation
                    ...
                    if dim == 3:
                        coord_list.extend(
                            [np.min(seg_ixs[:, 3]) - 1, np.max(seg_ixs[:, 3]) + 1]
                        )
                ...
                if class_specific_seg_flag:
                    out_seg[b][data_dict["seg"][b] == rix + 1] = (
                        data_dict["class_target"][b][rix] + 1
                    )
...
def resize_multichannel_image(multichannel_image, new_shape, order=3):
    ...
    for i in range(multichannel_image.shape[0]):
        result[i] = resize(
            multichannel_image[i].astype(float),
            new_shape,
            order,
            clip=True,
            anti_aliasing=False,
...
def mask_random_square(
    ...
    if img.ndim == 2:
        ...
        img[h_start : (h_start + square_size), w_start : (w_start + square_size)] = (
            rnd_n_val
        )
    elif img.ndim == 3:
