...
def maybe_download_and_prepare_cifar(target_dir, cifar=10):
    ...
    if not os.path.isfile(
        os.path.join(target_dir, "cifar%d_test_data.npz" % cifar)
    ) or not os.path.isfile(
        os.path.join(target_dir, "cifar%d_training_data.npz" % cifar)
    ):
        print("downloading CIFAR%d..." % cifar)
        urlretrieve(
            "http://www.cs.toronto.edu/~kriz/cifar-%d-python.tar.gz" % cifar,
            join(target_dir, "cifar-%d-python.tar.gz" % cifar),
        )

        tar = tarfile.open(
            os.path.join(target_dir, "cifar-%d-python.tar.gz" % cifar), "r:gz"
        )
        ...
        for batch in range(1, 6):
            loaded = unpickle(
                os.path.join(
                    target_dir, "cifar-%d-batches-py" % cifar, "data_batch_%d" % batch
                )
            )
            data.append(
                loaded[b"data"]
                .reshape((loaded[b"data"].shape[0], 3, 32, 32))
                .astype(np.uint8)
            )
        ...
        test = unpickle(
            os.path.join(target_dir, "cifar-%d-batches-py" % cifar, "test_batch")
        )
        data = (
            test[b"data"].reshape((test[b"data"].shape[0], 3, 32, 32)).astype(np.uint8)
        )
...
class CifarDataset(Dataset):
    ...
    def __getitem__(self, item):
        data_dict = {
            "data": self.data[item : item + 1].astype(np.float32),
            "labels": self.labels[item],
            "filenames": self.filenames[item],
        }
...
class HighPerformanceCIFARLoader(DataLoader):
    def __init__(
        ...
        super(HighPerformanceCIFARLoader, self).__init__(
            data,
            batch_size,
            num_threads_in_multithreaded,
            seed_for_shuffle,
