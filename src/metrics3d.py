...
def update_metrics_3D(metrics, pred: Tensor, gt: Tensor, patient_id: str, classes, metric_types) -> dict:
    metric_functions = {
        "dice": ("Dice", lambda p, g: our_dice_batch(p, g)),
        "sensitivity": ("Sensitivity", lambda p, g: Sensitivity_Specifity_metrics(p, g)[0]),
        "specificity": ("Specificity", lambda p, g: Sensitivity_Specifity_metrics(p, g)[1]),
    ...
    for metric_type in metric_types:
        if metric_type in metric_functions:
            ...
            metrics = append_3d_metrics(metrics, patient_id, classes, metric_name, results)
...
def Sensitivity_Specifity_metrics(pred_seg: Tensor, gt_seg: Tensor):
    # True positives
    true_positives = (pred_seg * gt_seg).sum(dim=(1, 2, 3)) # Only sum over the height and width (pixel dimensions)
