...
class CustomPredictor(nnUNetPredictor):
    def predict_single_npy_array(
        ...
        with torch.no_grad():
            ...
            segmentation = (
                self.convert_predicted_logits_to_segmentation_with_correct_shape(
                    predicted_logits, image_properties
                )
            )
        return segmentation
    ...
    def predict_preprocessed_image(self, image):
        ...
        for p in self.list_of_parameters:
            ...
            self.network.load_state_dict(
                torch.load(p, map_location=compute_device)["network_weights"]
            )
            with torch.autocast(self.device.type, enabled=True):
                for sl in tqdm(slicers, disable=not self.allow_tqdm):
                    pred = self._internal_maybe_mirror_and_predict(
                        data[sl][None].to(compute_device)
                    )[0].to(predicted_logits_device)
                    pred /= pred.max() / 100
    ...
    def convert_predicted_logits_to_segmentation_with_correct_shape(
        ...
        spacing_transposed = [
            props["spacing"][i] for i in self.plans_manager.transpose_forward
        ]
        current_spacing = (
            self.configuration_manager.spacing
            if len(self.configuration_manager.spacing)
            == len(props["shape_after_cropping_and_before_resampling"])
            else [spacing_transposed[0], *self.configuration_manager.spacing]
        )
        predicted_logits = self.configuration_manager.resampling_fn_probabilities(
            predicted_logits,
            props["shape_after_cropping_and_before_resampling"],
            current_spacing,
            [props["spacing"][i] for i in self.plans_manager.transpose_forward],
        )
        ...
        segmentation_reverted_cropping = np.zeros(
            props["shape_before_cropping"],
            dtype=np.uint8
            if len(self.label_manager.foreground_labels) < 255
            else np.uint16,
        )
        ...
        segmentation_reverted_cropping = segmentation_reverted_cropping.transpose(
            self.plans_manager.transpose_backward
        )
...
def predict_semseg(im, prop, semseg_trained_model, semseg_folds):
    ...
    semseg_pred = pred_semseg.predict_single_npy_array(im, prop, None)
    torch.cuda.empty_cache()
    gc.collect()
    return semseg_pred
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input_folder", type=Path, default="/input/images/cbct/"
    )
    parser.add_argument(
        "-o",
        "--output_folder",
    ...
    input_files = list(args.input_folder.glob("*.nii.gz")) + list(
        args.input_folder.glob("*.mha")
    )
