...
from utils import class2one_hot, probs2one_hot, save_images, probs2class
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm.tqdm(loader)):
            ...
            if args.save_png:
                ...
                save_directory = Path(
                    f"results_metrics/{args.model}/metrics2d/{str(args.from_checkpoint)[:-3]}"
                )
                save_directory.mkdir(parents=True, exist_ok=True)
                mult: int = 63 if K == 5 else (255 / (K - 1))
                save_images(
                    pred_class * mult, stems, save_directory / "png_predictions"
                )
        ...
        if args.save_png:
            ...
            print(
                "Metrics will not be stored as we might not have the ground truth for all the slices"
            )
        else:
            # Save the metrics in pickle format
            save_directory = Path(
                f"results_metrics/{args.model}/metrics2d/{args.from_checkpoint[:-3]}"
            )
            save_directory.mkdir(parents=True, exist_ok=True)
...
def main():
    ...
    parser.add_argument(
        "--save_png", action="store_true", help="Save the predictions as PNG"
    )
