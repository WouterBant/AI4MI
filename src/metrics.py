...
def print_store_metrics(metrics, destination):
    ...
    for metric in metrics.keys():
        ...
        formatted_metrics = [
            f"{avg:.4f} ({std:.4f})" for avg, std in zip(avg_metric, std_metric)
        ]
...
def update_metrics_2D(
    metrics, pred: Tensor, gt: Tensor, stems, classes, metric_types
) -> dict:
    ...
    if "dice" in metric_types:
        ...
        metrics = append_metrics(
            metrics, patients, slices, classes, metric_name, results
        )
...
def total_hausdorff_distance(pred_tensor, gt_tensor):
    ...
    for class_idx in range(gt_tensor.shape[1]):
        for sample_idx in range(gt_tensor.shape[0]):
            ...
            hausdorf_metrics[sample_idx, class_idx] = calculate_hausdorff_distance(
                ground_truth_img, prediction_img
            )
